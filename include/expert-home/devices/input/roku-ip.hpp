...
{
  boost::shared_ptr<struct http_task> http_task
    (new struct http_task(acceptor->get_io_service()));
...
struct roku_ip
{
  roku_ip(boost::asio::io_service& service, std::string listen_ip
    ...
    , acceptor_socket(service, boost::asio::ip::tcp::v4())
  ...
  void watch(std::function<void(std::string, std::vector<argument_variant>)> callback)
  {
    ...
    roku_detail::asynchronous_http(&acceptor);
  }

  void change_input(std::string const& )
  {
  }

  boost::asio::ip::udp::socket ssdp_socket;
  boost::asio::ip::tcp::socket acceptor_socket;
