...
#include <expert-home/calaos/lua_camera.hpp>
#include <json.hpp>
...
void get_home(nlohmann::json data, std::string msg_id, http_communication::client& client, lua_State* L)
{
  ...
  for(luabind::iterator iterator (cameras_lua), end; iterator != end; ++iterator)
  {
    std::cout << "looping" << std::endl;
    lua_camera lua_camera(*iterator);
    std::string id;
    if(lua_camera.has_id())
      id = lua_camera.id();
    else
    {
      iterator.key().push(L);
      ...
      id.insert(id.end(), p, p+len);
    }
    cameras.push_back
      (nlohmann::json
       ({
         {"id", id}
         , {"name", lua_camera.has_name() ? lua_camera.name() : id}
         , {"type", "string"}
         , {"ptz", lua_camera.has_ptz() ? "true" : "false"}
       }));
  }
  // cameras.push_back
  //   (nlohmann::json
